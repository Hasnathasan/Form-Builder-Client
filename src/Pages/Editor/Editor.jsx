...
const Editor = () => {
  ...
  return (
    <div>
      ...
      <form  className=" max-w-6xl mx-auto border-2 p-6 rounded-lg border-blue-500" onChange={handleSubmit(onSubmit)}>
        ...
        <div className="mb-20">
          ...
          <div className="flex gap-40">
            ...
            <div className="">
              ...
              <div className=" flex flex-col gap-2">
                {categories.length === 0 ? <h1 className="text-xl font-bold">Write a Category First</h1> : categories.map((category, index) => {
                  if (category !== "") {
                    return (
                      <div className="flex gap-20 justify-between items-start" key={index}>
                        <input
                        {...register(`item${index + 1}`, { required: false })}
                          className=" outline-none border border-gray-400 p-2 rounded"
                          type="text"
                          placeholder={"Item" + (index + 1)}
                        />
                        <select {...register(`cateSelected${index + 1}`, { required: false })} className=" flex flex-col gap-2">
